...
def preprocessing_fn(inputs):
    ...
    outputs[USER_ID_KEY] = tft.compute_and_apply_vocabulary(
        inputs[USER_ID_KEY], vocab_filename=USER_ID_KEY, num_oov_buckets=1
    )

    # Movie ID: Create vocabulary and map to integers
    outputs[MOVIE_ID_KEY] = tft.compute_and_apply_vocabulary(
        inputs[MOVIE_ID_KEY], vocab_filename=MOVIE_ID_KEY, num_oov_buckets=1
    )

    # Age: Bucketize into 6 bins (quantile boundaries)
    outputs[AGE_KEY] = tft.bucketize(inputs[AGE_KEY], num_buckets=6, epsilon=0.01)
    # Age: fixed buckets
    ...
    outputs[GENDER_KEY] = tft.compute_and_apply_vocabulary(
        inputs[GENDER_KEY], vocab_filename=GENDER_KEY, num_oov_buckets=1
    )

    # Occupation: Create vocabulary (21 categories)
    outputs[OCCUPATION_KEY] = tft.compute_and_apply_vocabulary(
        inputs[OCCUPATION_KEY], vocab_filename=OCCUPATION_KEY, num_oov_buckets=1
    )
    ...
    outputs[GENRES_KEY] = tft.compute_and_apply_vocabulary(
        genre_tokens, vocab_filename=GENRES_KEY, num_oov_buckets=1
    )
